...
class CrossModalPred(pl.LightningModule):
    ...
    def __init__(
        ...
        input_dims = [
            len(dataset.features[self.input_layers[i]])
        ...
        self.FC_mean = nn.Linear(
            len(self.input_layers) * config["latent_dim"], config["latent_dim"]
        )
        ...
        output_dims = [
            len(dataset.features[self.output_layers[i]])
    ...
    def training_step(self, train_batch, batch_idx, log=True):
        ...
        mmd_loss_list = [
            self.MMD_loss(z.shape[1], z, x_hat_list[i], x_list_output[i])
            for i in range(len(self.output_layers))
        ]
