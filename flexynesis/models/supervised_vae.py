...
class supervised_vae(pl.LightningModule):
    ...
    def __init__(
        ...
        self.FC_mean = nn.Linear(
            len(layers) * config["latent_dim"], config["latent_dim"]
        )
    ...
    def training_step(self, train_batch, batch_idx, log=True):
        ...
        mmd_loss_list = [
            self.MMD_loss(z.shape[1], z, x_hat_list[i], x_list[i])
            for i in range(len(layers))
        ]
    ...
    def transform(self, dataset):
        ...
        all_latent_representations = (
            []
        )  # Initialize a list to collect all batch latent representations
        ...
        for batch in dataloader:
            dat, _, samples = batch
            x_list = [
                dat[x].to(device) for x in dat.keys()
            ]  # Prepare the data batch for processing

            # Perform the forward pass and extract the latent representation
            latent_representation = (
                self.forward(x_list)[1].detach().cpu().numpy()
            )  # Index [1] assumes second return is the latent rep

            all_latent_representations.append(
                latent_representation
            )  # Store the batch's latent representation
    ...
    def compute_feature_importance(
        ...
        print(
            "[INFO] Computing feature importance for variable:",
            target_var,
            "on device:",
            device,
        )
