...
class HyperparameterTuning:
    ...
    def __init__(
        ...
        )  # use torch data loader by default

        if self.model_class.__name__ == "MultiTripletNetwork":
            self.loader_dataset = TripletMultiOmicDataset(
                self.dataset, self.target_variables[0]
            )
        ...
        if config_path:
            ...
            if self.config_name in external_config:
                ...
                raise ValueError(
                    f"'{self.config_name}' not found in the provided config file."
                )
        else:
            if self.config_name in search_spaces:
                ...
                raise ValueError(
                    f"'{self.config_name}' not found in the default config."
                )
    ...
    def objective(self, params, current_step, total_steps, full_train=False):
        ...
        if full_train:
            ...
            trainer, _ = self.setup_trainer(
                params, current_step, total_steps, full_train=True
            )
            ...
            if self.use_cv:  # if the user asks for cross-validation
                ...
                train_subset, val_subset = random_split(
                    self.loader_dataset, [num_train, num_val]
                )
                # create single split format similar to KFold
                split_iterator = [
                    (list(train_subset.indices), list(val_subset.indices))
                ]
            i = 1
            ...
            for train_index, val_index in split_iterator:
                ...
                model = self.model_class(**model_args)
                trainer, early_stop_callback = self.setup_trainer(
                    params, current_step, total_steps
                )
    ...
    def perform_tuning(self, hpo_patience=0):
        ...
        with tqdm(total=self.n_iter, desc="Tuning Progress") as pbar:
            for i in range(self.n_iter):
                ...
                if self.use_cv:
                    print(
                        f"[INFO] average 5-fold cross-validation loss {loss} for params: {suggested_params_dict}"
                    )
            ...
            for config in self.freeze_configs:
                ...
                for fold in range(self.n_splits):
                    model_copy = copy.deepcopy(
                        self.original_model
                    )  # Deep copy the model for each fold
                    self.model = model_copy
                    self.apply_freeze_config(
                        config
                    )  # try freezing different components
                    ...
                    val_loss = trainer.validate(
                        self, dataloaders=self.val_dataloader(), verbose=False
                    )
                    fold_losses.append(
                        val_loss[0]["val_loss"]
